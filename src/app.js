...
// Allowed origins (dev, Capacitor, prod, Vercel preview)
const ALLOW_LIST = [
  'http://localhost:8100',  // Ionic dev
  'capacitor://localhost',  // Android/iOS Capacitor
  'ionic://localhost',
  'https://learnasl.vercel.app', // your prod domain (example)
  // Add your custom domain if mapped, e.g. 'https://learnasl.example.com'
];

// Allow preview deployments like https://learnasl-somehash.vercel.app
const ALLOW_REGEX = [/^https:\/\/learnasl-[a-z0-9-]+\.vercel\.app$/i];

const corsOptions = {
  origin(origin, cb) {
    if (!origin) return cb(null, true); // curl/Postman
    const ok = ALLOW_LIST.includes(origin) || ALLOW_REGEX.some(rx => rx.test(origin));
    return ok ? cb(null, true) : cb(new Error('Not allowed by CORS'));
  },
  methods: ['GET', 'HEAD', 'OPTIONS'],
  allowedHeaders: ['Range', 'Accept', 'Origin'],
  maxAge: 86400,
};
